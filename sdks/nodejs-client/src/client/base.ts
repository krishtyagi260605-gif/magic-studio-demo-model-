...
import { FileUploadError, ValidationError } from "../errors/magicstudio-error";
import type { SdkFormData } from "../http/form-data";
import { isFormData } from "../http/form-data";

const toConfig = (
  init: string | MagicStudioClientConfig,
  baseUrl?: string
): MagicStudioClientConfig => {
...
export class MagicStudioClient {
  protected http: HttpClient;

  constructor(config: string | MagicStudioClientConfig | HttpClient, baseUrl?: string) {
  ...
  getRoot(): Promise<MagicStudioResponse<JsonObject>> {
  ...
  getApplicationParameters(user?: string): Promise<MagicStudioResponse<JsonObject>> {
    if (user) {
  ...
  async getParameters(user?: string): Promise<MagicStudioResponse<JsonObject>> {
    return this.getApplicationParameters(user);
  }

  getMeta(user?: string): Promise<MagicStudioResponse<JsonObject>> {
    if (user) {
      ensureNonEmptyString(user, "user");
    }
    return this.http.request({
  ...
  getInfo(user?: string): Promise<MagicStudioResponse<JsonObject>> {
    if (user) {
      ensureNonEmptyString(user, "user");
    }
    return this.http.request({
  ...
  getSite(user?: string): Promise<MagicStudioResponse<JsonObject>> {
    if (user) {
      ensureNonEmptyString(user, "user");
    }
    return this.http.request({
  ...
  fileUpload(form: unknown, user: string): Promise<MagicStudioResponse<JsonObject>> {
    if (!isFormData(form)) {
  ...
  ): Promise<MagicStudioResponse<Buffer>> {
  ...
  audioToText(form: unknown, user: string): Promise<MagicStudioResponse<JsonObject>> {
    if (!isFormData(form)) {
  ...
  ): Promise<MagicStudioResponse<Buffer> | BinaryStream>;
  textToAudio(
    text: string,
    user: string,
    streaming?: boolean,
