...
export const DEFAULT_BASE_URL = "https://api.magicstudio.ai/v1";
...
export type MagicStudioRequestFile = JsonObject;
...
export type MagicStudioClientConfig = {
  apiKey: string;
  baseUrl?: string;
  timeout?: number;
  maxRetries?: number;
...
export type MagicStudioResponse<T> = {
  data: T;
  status: number;
  headers: Headers;
  requestId?: string;
...
export type MagicStudioStream<T = unknown> = AsyncIterable<StreamEvent<T>> & {
  data: Readable;
  status: number;
  headers: Headers;
  requestId?: string;
