...
const DEFAULT_USER_AGENT = "magicstudio-client-node";
...
export type HttpClientSettings = Required<
  Omit<MagicStudioClientConfig, "apiKey">
> & {
  apiKey: string;
};
...
const normalizeSettings = (config: MagicStudioClientConfig): HttpClientSettings => ({
...
const mapTransportError = (
  ...
  if (error instanceof MagicStudioError) {
    return error;
  }
...
export class HttpClient {
  ...
  constructor(config: MagicStudioClientConfig) {
  ...
  >(options: RequestOptions<TResponseType>): Promise<MagicStudioResponse<T>> {
  ...
  async requestStream<T>(options: RequestOptions): Promise<MagicStudioStream<T>> {
  ...
  async requestRaw<TResponseType extends HttpResponseType = "json">(
    ...
    if (enableLogging) {
      console.info(`magicstudio-client-node request ${method} ${url}`);
    }
    ...
    while (true) {
      ...
      try {
        ...
        if (enableLogging) {
          console.info(
            `magicstudio-client-node response ${response.status} ${method} ${url}`
          );
        }
        ...
        if (enableLogging) {
          console.info(
            `magicstudio-client-node retry ${attempt + 1} in ${delay}ms for ${method} ${url}`
          );
        }
        attempt += 1;
        await sleep(delay);
