...
import { BASE_URL, ChatClient, MagicStudioClient, WorkflowClient, routes } from "./index";
...
describe("Client", () => {
  ...
  it("creates a client with default settings", () => {
    const magicstudioClient = new MagicStudioClient("test");

    expect(magicstudioClient.getHttpClient().getSettings()).toMatchObject({
      apiKey: "test",
      baseUrl: BASE_URL,
      timeout: 60,
    });
  ...
  it("updates the api key", () => {
    const magicstudioClient = new MagicStudioClient("test");
    magicstudioClient.updateApiKey("test2");

    expect(magicstudioClient.getHttpClient().getSettings().apiKey).toBe("test2");
  });
});
...
describe("Send Requests", () => {
  ...
  it("makes a successful request to the application parameter route", async () => {
    ...
    const response = await magicstudioClient.sendRequest(method, endpoint);
    ...
    expect(init.headers).toMatchObject({
      ...
      "User-Agent": "magicstudio-client-node",
    });
  });
  ...
  it("uses the getMeta route configuration", async () => {
    ...
    await magicstudioClient.getMeta("end-user");
...
describe("File uploads", () => {
  ...
  it("does not override multipart boundary headers for legacy FormData", async () => {
    ...
    await magicstudioClient.fileUpload(form, "end-user");
