 ...
 * the magicStudioChatbotConfig should be defined in the html file before this script is included
 * the magicStudioChatbotConfig should contain the token of the chatbot
...
(function () {
  ...
  const configKey = "magicStudioChatbotConfig";
  const buttonId = "magic-studio-chatbot-bubble-button";
  const iframeId = "magic-studio-chatbot-bubble-window";
  ...
  const originalIframeStyleText = `
    ...
    top: unset;
    right: var(--${buttonId}-right, 1rem); /* Align with magic-studio-chatbot-bubble-button. */
    bottom: var(--${buttonId}-bottom, 1rem); /* Align with magic-studio-chatbot-bubble-button. */
    left: unset;
    width: 24rem;
  ...
  async function embedChatbot() {
    ...
    function createIframe() {
      ...
      iframe.title = "Magic Studio chatbot bubble window";
      iframe.id = iframeId;
    ...
    window.addEventListener('message', (event) => {
      ...
      if (event.data.type === 'magic-studio-chatbot-iframe-ready') {
      ...
      if (event.data.type === 'magic-studio-chatbot-expand-change') {
        toggleExpand();
      }
    });
