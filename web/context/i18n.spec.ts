...
describe('useDocLink', () => {
  ...
  describe('Base URL handling', () => {
    ...
    it('should remove trailing slash from base URL', () => {
      ...
      const url = result.current('/magic-studio/getting-started/introduction')
      expect(url).toBe('https://docs.dify.ai/en/magic-studio/getting-started/introduction')
    })
  ...
  describe('Path handling', () => {
    it('should handle path parameter', () => {
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/magic-studio/getting-started/introduction')
      expect(url).toBe(`${defaultDocBaseUrl}/en/magic-studio/getting-started/introduction`)
    })
  ...
  describe('PathMap handling', () => {
    it('should use path from pathMap when locale matches', () => {
      ...
      const pathMap: DocPathMap = {
        'zh-Hans': '/magic-studio/getting-started/introduction',
        'en-US': '/magic-studio/getting-started/quick-start',
      }

      const { result } = renderHook(() => useDocLink())
      const url = result.current('/magic-studio/getting-started/quick-start' as DocPathWithoutLang, pathMap)
      expect(url).toBe(`${defaultDocBaseUrl}/zh/magic-studio/getting-started/introduction`)
    })
    ...
    it('should use default path when locale not in pathMap', () => {
      ...
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/magic-studio/getting-started/quick-start' as DocPathWithoutLang, pathMap)
      expect(url).toBe(`${defaultDocBaseUrl}/ja/magic-studio/getting-started/quick-start`)
    })

    it('should handle undefined pathMap', () => {
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/magic-studio/getting-started/introduction', undefined)
      expect(url).toBe(`${defaultDocBaseUrl}/en/magic-studio/getting-started/introduction`)
    })
  })
  ...
  describe('Edge Cases', () => {
    it('should handle path with anchor', () => {
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/magic-studio/getting-started/introduction#overview' as DocPathWithoutLang)
      expect(url).toBe(`${defaultDocBaseUrl}/en/magic-studio/getting-started/introduction#overview`)
    })

    it('should handle multiple calls with same hook instance', () => {
      const { result } = renderHook(() => useDocLink())
      const url1 = result.current('/magic-studio/getting-started/introduction')
      const url2 = result.current('/magic-studio/getting-started/quick-start')
      expect(url1).toBe(`${defaultDocBaseUrl}/en/magic-studio/getting-started/introduction`)
      expect(url2).toBe(`${defaultDocBaseUrl}/en/magic-studio/getting-started/quick-start`)
    })
  })
})
