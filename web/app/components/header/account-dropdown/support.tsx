...
import { BRAND_CONTACT_MAILTO, BRAND_REPO_URL } from '@/constants/brand'
...
export default function Support({ closeAccountDropdown }: SupportProps) {
  ...
  const { userProfile, magicStudioVersionInfo } = useAppContext()
        ...
        <DropdownMenuGroup className="py-1">
          ...
          {hasDedicatedChannel && !hasZendeskWidget && (
            <DropdownMenuLinkItem
              className="justify-between"
              href={mailToSupport(userProfile.email, plan.type, magicStudioVersionInfo?.current_version, SUPPORT_EMAIL_ADDRESS)}
