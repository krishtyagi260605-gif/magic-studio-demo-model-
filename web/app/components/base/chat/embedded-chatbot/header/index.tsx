...
const Header: FC<IHeaderProps> = ({
  ...
  const handleMessageReceived = useCallback((event: MessageEvent) => {
    ...
    if (!currentParentOrigin && event.data.type === 'magic-studio-chatbot-config') {
      currentParentOrigin = event.origin
      setParentOrigin(event.origin)
    }
    if (event.origin !== currentParentOrigin)
      return
    if (event.data.type === 'magic-studio-chatbot-config')
  ...
  useEffect(() => {
    ...
    window.parent.postMessage({ type: 'magic-studio-chatbot-iframe-ready' }, targetOrigin)
