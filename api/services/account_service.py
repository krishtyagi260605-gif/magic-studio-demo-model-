...
def _try_join_enterprise_default_workspace(account_id: str) -> None:
    ...
    if not magic_studio_config.ENTERPRISE_ENABLED:
        return
...
REFRESH_TOKEN_EXPIRY = timedelta(days=magic_studio_config.REFRESH_TOKEN_EXPIRE_DAYS)
...
class AccountService:
    ...
    def get_account_jwt_token(account: Account) -> str:
        exp_dt = datetime.now(UTC) + timedelta(minutes=magic_studio_config.ACCESS_TOKEN_EXPIRE_MINUTES)
        exp = int(exp_dt.timestamp())
        payload = {
            "user_id": account.id,
            "exp": exp,
            "iss": magic_studio_config.EDITION,
    ...
    def create_account(
        ...
        if magic_studio_config.BILLING_ENABLED and BillingService.is_email_in_freeze(email):
    ...
    def add_login_error_rate_limit(email: str):
        ...
        if count is None:
            count = 0
        count = int(count) + 1
        redis_client.setex(key, magic_studio_config.LOGIN_LOCKOUT_DURATION, count)
    ...
    def add_forgot_password_error_rate_limit(email: str):
        ...
        if count is None:
            count = 0
        count = int(count) + 1
        redis_client.setex(key, magic_studio_config.FORGOT_PASSWORD_LOCKOUT_DURATION, count)
    ...
    def add_email_register_error_rate_limit(email: str) -> None:
        ...
        if count is None:
            count = 0
        count = int(count) + 1
        redis_client.setex(key, magic_studio_config.EMAIL_REGISTER_LOCKOUT_DURATION, count)
    ...
    def add_change_email_error_rate_limit(email: str):
        ...
        if count is None:
            count = 0
        count = int(count) + 1
        redis_client.setex(key, magic_studio_config.CHANGE_EMAIL_LOCKOUT_DURATION, count)
    ...
    def add_owner_transfer_error_rate_limit(email: str):
        ...
        if count is None:
            count = 0
        count = int(count) + 1
        redis_client.setex(key, magic_studio_config.OWNER_TRANSFER_LOCKOUT_DURATION, count)
    ...
    def is_email_send_ip_limit(ip_address: str):
        ...
        if current_minute_count > magic_studio_config.EMAIL_SEND_IP_LIMIT_PER_MINUTE:
...
class TenantService:
    ...
    def create_tenant_member(tenant: Tenant, account: Account, role: str = "normal") -> TenantAccountJoin:
        ...
        db.session.commit()
        if magic_studio_config.BILLING_ENABLED:
...
class RegisterService:
    ...
    def setup(cls, email: str, name: str, password: str, ip_address: str, language: str | None):
        ...
        try:
            ...
            dify_setup = DifySetup(version=magic_studio_config.project.version)
    ...
    def generate_invite_token(cls, tenant: Tenant, account: Account) -> str:
        ...
        expiry_hours = magic_studio_config.INVITE_EXPIRY_HOURS
