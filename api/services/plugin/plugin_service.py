...
class PluginService:
    ...
    def get_plugin_icon_url(cls, tenant_id: str, filename: str) -> str:
        url_prefix = (
            URL(magic_studio_config.CONSOLE_API_URL or "/") / "console" / "api" / "workspaces" / "current" / "plugin" / "icon"
        )
    ...
    def upgrade_plugin_with_marketplace(
        ...
        """
        if not magic_studio_config.MARKETPLACE_ENABLED:
    ...
    def upload_pkg_from_github(
        ...
        pkg = download_with_size_limit(
            f"https://github.com/{repo}/releases/download/{version}/{package}", magic_studio_config.PLUGIN_MAX_PACKAGE_SIZE
        )
        features = FeatureService.get_system_features()
