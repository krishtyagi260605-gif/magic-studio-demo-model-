...
class ScheduleService:
    ...
    def get_scheduled_tasks(
        ...
        """
        Get all scheduled tasks for a tenant, optionally filtered by app.
        """
        stmt = select(WorkflowSchedulePlan).where(WorkflowSchedulePlan.tenant_id == tenant_id)
        if app_id:
            stmt = stmt.where(WorkflowSchedulePlan.app_id == app_id)
        
        stmt = stmt.order_by(WorkflowSchedulePlan.next_run_at.asc())
