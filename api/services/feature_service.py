...
class SystemFeatureModel(BaseModel):
    ...
    max_plugin_package_size: int = magic_studio_config.PLUGIN_MAX_PACKAGE_SIZE
...
class FeatureService:
    ...
    def get_features(cls, tenant_id: str) -> FeatureModel:
        ...
        if magic_studio_config.BILLING_ENABLED and tenant_id:
            cls._fulfill_params_from_billing_api(features, tenant_id)

        if magic_studio_config.ENTERPRISE_ENABLED:
    ...
    def _resolve_human_input_email_delivery_enabled(cls, *, features: FeatureModel, tenant_id: str | None) -> bool:
        if magic_studio_config.ENTERPRISE_ENABLED or not magic_studio_config.BILLING_ENABLED:
            return True
        if not tenant_id:
            return False
    ...
    def get_system_features(cls, is_authenticated: bool = False) -> SystemFeatureModel:
        ...
        if magic_studio_config.MARKETPLACE_ENABLED:
    ...
    def _fulfill_system_params_from_env(cls, system_features: SystemFeatureModel):
        system_features.enable_email_code_login = magic_studio_config.ENABLE_EMAIL_CODE_LOGIN
        system_features.enable_email_password_login = magic_studio_config.ENABLE_EMAIL_PASSWORD_LOGIN
        system_features.enable_social_oauth_login = magic_studio_config.ENABLE_SOCIAL_OAUTH_LOGIN
        system_features.is_allow_register = magic_studio_config.ALLOW_REGISTER
        system_features.is_allow_create_workspace = magic_studio_config.ALLOW_CREATE_WORKSPACE
        system_features.is_email_setup = magic_studio_config.MAIL_TYPE is not None and magic_studio_config.MAIL_TYPE != ""
        system_features.trial_models = cls._fulfill_trial_models_from_env()
        system_features.enable_trial_app = magic_studio_config.ENABLE_TRIAL_APP
        system_features.enable_explore_banner = magic_studio_config.ENABLE_EXPLORE_BANNER
    ...
    def _fulfill_trial_models_from_env(cls) -> list[str]:
        return [
            ...
            if (
                getattr(magic_studio_config, f"HOSTED_{provider.config_key}_PAID_ENABLED", False)
                and getattr(magic_studio_config, f"HOSTED_{provider.config_key}_TRIAL_ENABLED", False)
            )
        ]

    @classmethod
    def _fulfill_params_from_env(cls, features: FeatureModel):
        features.can_replace_logo = magic_studio_config.CAN_REPLACE_LOGO
        features.model_load_balancing_enabled = magic_studio_config.MODEL_LB_ENABLED
        features.dataset_operator_enabled = magic_studio_config.DATASET_OPERATOR_ENABLED
        features.education.enabled = magic_studio_config.EDUCATION_ENABLED
