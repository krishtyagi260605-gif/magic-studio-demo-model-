...
class ApiTokenCache:
    ...
    def _serialize_token(api_token: Any) -> bytes:
        ...
        cached = CachedApiToken(
            ...
            workflow_id=str(api_token.workflow_id) if api_token.workflow_id else None,
            last_used_at=api_token.last_used_at,
            created_at=api_token.created_at,
            rate_limit_per_minute=api_token.rate_limit_per_minute,
            token_budget=api_token.token_budget,
            used_tokens=api_token.used_tokens,
        )
...
def record_token_consumption(api_token_id: str, tokens: int) -> None:
    """
    Record token consumption for an API token.
    Increments the used_tokens count in Redis for immediate guardrail enforcement.
    """
    if tokens <= 0:
        return
    try:
        # Increment in Redis for fast guardrail check
        consumption_key = f"api_token_consumption:{api_token_id}"
        redis_client.incrby(consumption_key, tokens)
        redis_client.expire(consumption_key, 86400)  # 1 day TTL for sync window
        
        # In a real production environment, a background task would periodically
        # sync these Redis counters to the DB's used_tokens field.
        # For Magic Studio MVP, we assume the background sync is handled.
    except Exception as e:
        logger.warning("Failed to record token consumption: %s", e)
