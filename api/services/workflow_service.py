...
class WorkflowService:
    ...
    def get_published_workflow(self, app_model: App) -> Workflow | None:
        ...
        # 2. Fallback: find any workflow marked as 'production' for this app
        workflow = db.session.scalar(
            select(Workflow)
            .where(
                Workflow.tenant_id == app_model.tenant_id,
    ...
    def publish_workflow(
        ...
        *,
        ...
        # Magic Studio: Default to staging for new versions
        environment = "staging"
    ...
    def publish_to_production(self, app_model: App, workflow_id: str) -> Workflow:
        """
        Magic Studio: Promotes a specific workflow version to the Production environment.
        Sets all other versions to Staging.
        """
        ...
        # Set all other versions to staging
        db.session.execute(
            sa.update(Workflow)
            .where(Workflow.app_id == app_model.id, Workflow.id != workflow_id)
