...
def get_oauth_providers():
    with current_app.app_context():
        notion_oauth = NotionOAuth(
            client_id=magic_studio_config.NOTION_CLIENT_ID or "",
            client_secret=magic_studio_config.NOTION_CLIENT_SECRET or "",
            redirect_uri=magic_studio_config.CONSOLE_API_URL + "/console/api/oauth/data-source/callback/notion",
        )
...
class OAuthDataSource(Resource):
    ...
    def get(self, provider: str):
        ...
        if magic_studio_config.NOTION_INTEGRATION_TYPE == "internal":
            internal_secret = magic_studio_config.NOTION_INTERNAL_SECRET
...
class OAuthDataSourceCallback(Resource):
    ...
    def get(self, provider: str):
        ...
        if "code" in request.args:
            ...
            return redirect(f"{magic_studio_config.CONSOLE_WEB_URL}?type=notion&code={code}")
        elif "error" in request.args:
            error = request.args.get("error")

            return redirect(f"{magic_studio_config.CONSOLE_WEB_URL}?type=notion&error={error}")
        else:
            return redirect(f"{magic_studio_config.CONSOLE_WEB_URL}?type=notion&error=Access denied")
