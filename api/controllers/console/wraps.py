...
def only_edition_self_hosted[**P, R](view: Callable[P, R]) -> Callable[P, R]:
    @wraps(view)
    def decorated(*args: P.args, **kwargs: P.kwargs):
        if magic_studio_config.EDITION != "SELF_HOSTED":
            abort(404)
...
def setup_required[**P, R](view: Callable[P, R]) -> Callable[P, R]:
    ...
    def decorated(*args: P.args, **kwargs: P.kwargs) -> R:
        # check setup
        if magic_studio_config.EDITION == "SELF_HOSTED" and not db.session.scalar(select(DifySetup).limit(1)):
...
def annotation_import_rate_limit[**P, R](view: Callable[P, R]) -> Callable[P, R]:
    ...
    def decorated(*args: P.args, **kwargs: P.kwargs):
        ...
        if minute_count > magic_studio_config.ANNOTATION_IMPORT_RATE_LIMIT_PER_MINUTE:
            abort(
                429,
                f"Too many annotation import requests. Maximum {magic_studio_config.ANNOTATION_IMPORT_RATE_LIMIT_PER_MINUTE} "
        ...
        if hour_count > magic_studio_config.ANNOTATION_IMPORT_RATE_LIMIT_PER_HOUR:
            abort(
                429,
                f"Too many annotation import requests. Maximum {magic_studio_config.ANNOTATION_IMPORT_RATE_LIMIT_PER_HOUR} "
...
def annotation_import_concurrency_limit[**P, R](view: Callable[P, R]) -> Callable[P, R]:
    ...
    def decorated(*args: P.args, **kwargs: P.kwargs):
        ...
        if active_count >= magic_studio_config.ANNOTATION_IMPORT_MAX_CONCURRENT:
            abort(
                429,
                f"Too many concurrent import tasks. Maximum {magic_studio_config.ANNOTATION_IMPORT_MAX_CONCURRENT} "
