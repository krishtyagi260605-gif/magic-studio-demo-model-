...
def validate_app_token[**P, R](
    ...
    def decorator(view_func: Callable[P, R]) -> Callable[P, R]:
        ...
        def decorated_view(*args: P.args, **kwargs: P.kwargs) -> R:
            ...
            # Magic Studio: Workflow-scoped key enforcement
            if api_token.workflow_id:
                # Check if this is a workflow-related request (AppMode.WORKFLOW or AppMode.ADVANCED_CHAT)
                from models.model import AppMode
                if app_model.mode not in [AppMode.WORKFLOW, AppMode.ADVANCED_CHAT]:
                    raise Forbidden("This API key is restricted to a specific workflow.")
                
                # If a specific workflow_id is requested in the URL/Body, it MUST match
                requested_workflow_id = None
                if request.view_args and 'workflow_id' in request.view_args:
                    requested_workflow_id = request.view_args['workflow_id']
                elif request.is_json and request.get_json(silent=True) and 'workflow_id' in request.get_json():
                    requested_workflow_id = request.get_json().get('workflow_id')
                
                if requested_workflow_id and requested_workflow_id != api_token.workflow_id:
                    raise Forbidden(f"This API key is only authorized for workflow {api_token.workflow_id}.")

                # If no workflow_id is requested, Dify usually uses app_model.workflow_id (the published one).
                # We must ensure app_model.workflow_id matches the scoped key's workflow_id.
                if not requested_workflow_id and app_model.workflow_id != api_token.workflow_id:
                    raise Forbidden(f"This API key is only authorized for workflow {api_token.workflow_id}. Please use the specific workflow endpoint.")
...
def validate_and_get_api_token(scope: str | None = None):
    ...
    api_token = cast(ApiToken, cached_token) if cached_token else fetch_token_with_single_flight(auth_token, scope)

    # Magic Studio: Rate Limiting
    if api_token.rate_limit_per_minute:
        current_time = int(time.time() / 60)
        rate_limit_key = f"api_rate_limit:{api_token.id}:{current_time}"
        count = redis_client.incr(rate_limit_key)
        if count == 1:
            redis_client.expire(rate_limit_key, 60)
        if count > api_token.rate_limit_per_minute:
            raise Forbidden("Rate limit exceeded for this API key.")

    # Magic Studio: Cost Guardrails (Token Budget)
    if api_token.token_budget is not None and api_token.used_tokens >= api_token.token_budget:
        raise Forbidden("Token budget exceeded for this API key. Please top up in Magic Studio.")
