...
def test_publish_blocks_start_and_trigger_coexistence(
    ...
    monkeypatch.setattr("services.workflow_service.magic_studio_config", SimpleNamespace(BILLING_ENABLED=False))
...
def test_trigger_url_uses_config_base(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    original_url = getattr(magic_studio_config, "TRIGGER_URL", None)

    try:
        monkeypatch.setattr(magic_studio_config, "TRIGGER_URL", TEST_TRIGGER_URL)
        ...
        if original_url is not None:
            monkeypatch.setattr(magic_studio_config, "TRIGGER_URL", original_url)
