...
class TestEmailRegisterSendEmailApi:
    ...
    def test_send_email_normalizes_and_falls_back(
        ...
        with (
            patch("controllers.console.auth.email_register.magic_studio_config", SimpleNamespace(BILLING_ENABLED=True)),
            patch("controllers.console.wraps.magic_studio_config", SimpleNamespace(EDITION="CLOUD")),
