...
class TestMessagesCleanServiceIntegration:
    ...
    def mock_billing_enabled(self):
        ...
        with patch("services.retention.conversation.messages_clean_policy.magic_studio_config.BILLING_ENABLED", True):
            yield

    @pytest.fixture
    def mock_billing_disabled(self):
        """Mock BILLING_ENABLED to be False."""
        with patch("services.retention.conversation.messages_clean_policy.magic_studio_config.BILLING_ENABLED", False):
            yield
