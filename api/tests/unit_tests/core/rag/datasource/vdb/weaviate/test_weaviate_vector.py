...
class TestWeaviateVector(unittest.TestCase):
    ...
    @patch("core.rag.datasource.vdb.weaviate.weaviate_vector.magic_studio_config")
    @patch("core.rag.datasource.vdb.weaviate.weaviate_vector.weaviate")
    def test_create_collection_includes_doc_type_property(self, mock_weaviate_module, mock_magic_studio_config, mock_redis):
        ...
        # Mock magic_studio_config
        mock_magic_studio_config.WEAVIATE_TOKENIZATION = None
...
class TestWeaviateVectorFactory(unittest.TestCase):
    def test_init_vector_uses_existing_dataset_index_struct(self):
        ...
        with (
            patch.object(weaviate_vector_module.magic_studio_config, "WEAVIATE_ENDPOINT", "http://localhost:8080"),
            patch.object(weaviate_vector_module.magic_studio_config, "WEAVIATE_GRPC_ENDPOINT", "localhost:50051"),
            patch.object(weaviate_vector_module.magic_studio_config, "WEAVIATE_API_KEY", "api-key"),
            patch.object(weaviate_vector_module.magic_studio_config, "WEAVIATE_BATCH_SIZE", 88),
            patch(
    ...
    def test_init_vector_generates_collection_and_updates_index_struct(self):
        ...
        with (
            patch.object(weaviate_vector_module.magic_studio_config, "WEAVIATE_ENDPOINT", "http://localhost:8080"),
            patch.object(weaviate_vector_module.magic_studio_config, "WEAVIATE_GRPC_ENDPOINT", ""),
            patch.object(weaviate_vector_module.magic_studio_config, "WEAVIATE_API_KEY", None),
            patch.object(weaviate_vector_module.magic_studio_config, "WEAVIATE_BATCH_SIZE", 100),
            patch.object(
