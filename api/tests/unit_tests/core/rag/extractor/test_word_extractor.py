...
def test_extract_images_from_docx(monkeypatch):
    ...
    monkeypatch.setattr(we.magic_studio_config, "FILES_URL", "http://files.local", raising=False)
    monkeypatch.setattr(we.magic_studio_config, "STORAGE_TYPE", "local", raising=False)
...
def test_extract_images_from_docx_uses_internal_files_url():
    ...
    original_files_url = getattr(magic_studio_config, "FILES_URL", None)
    original_internal_files_url = getattr(magic_studio_config, "INTERNAL_FILES_URL", None)

    try:
        # Set both URLs - INTERNAL should take precedence
        magic_studio_config.FILES_URL = "http://external.example.com"
        magic_studio_config.INTERNAL_FILES_URL = "http://internal.docker:5001"
        ...
        base_url = magic_studio_config.INTERNAL_FILES_URL or magic_studio_config.FILES_URL
        ...
        magic_studio_config.FILES_URL = original_files_url
        magic_studio_config.INTERNAL_FILES_URL = original_internal_files_url
