...
class TestWorkflowEntryInit:
    def test_rejects_call_depth_above_limit(self):
        call_depth = workflow_entry.magic_studio_config.WORKFLOW_CALL_MAX_DEPTH + 1
    ...
    def test_applies_debug_and_observability_layers(self):
        ...
        with (
            patch.object(workflow_entry.magic_studio_config, "DEBUG", True),
            patch.object(workflow_entry.magic_studio_config, "ENABLE_OTEL", False),
        ...
        execution_limits_layer_cls.assert_called_once_with(
            max_steps=workflow_entry.magic_studio_config.WORKFLOW_MAX_EXECUTION_STEPS,
            max_time=workflow_entry.magic_studio_config.WORKFLOW_MAX_EXECUTION_TIME,
        )
