...
def test_resolve_upload_file_url_signs_internal_urls_and_supports_attachments(
    ...
    monkeypatch.setattr("core.app.workflow.file_runtime.magic_studio_config.SECRET_KEY", "unit-secret")
    monkeypatch.setattr("core.app.workflow.file_runtime.magic_studio_config.FILES_URL", "https://files.example.com")
    monkeypatch.setattr(
        "core.app.workflow.file_runtime.magic_studio_config.INTERNAL_FILES_URL",
...
def test_verify_preview_signature_validates_signature_and_expiration(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("core.app.workflow.file_runtime.time.time", lambda: 1700000000)
    monkeypatch.setattr("core.app.workflow.file_runtime.magic_studio_config.SECRET_KEY", "unit-secret")
    monkeypatch.setattr("core.app.workflow.file_runtime.magic_studio_config.FILES_ACCESS_TIMEOUT", 60)
    runtime = _build_runtime()
...
def test_runtime_helper_wrappers_delegate_to_config_and_io(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("core.app.workflow.file_runtime.magic_studio_config.MULTIMODAL_SEND_FORMAT", "url")
    runtime = _build_runtime()
