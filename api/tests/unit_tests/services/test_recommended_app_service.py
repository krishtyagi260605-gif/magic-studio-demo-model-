...
- **Mocking Strategy**: All external dependencies (magic_studio_config, RecommendAppRetrievalFactory)
...
class TestRecommendedAppServiceGetApps:
    ...
    @patch("services.recommended_app_service.magic_studio_config", autospec=True)
...
def _mock_factory_for_apps(
    ...
    *,
    ...
    monkeypatch.setattr(service_module.magic_studio_config, "HOSTED_FETCH_APP_TEMPLATES_MODE", mode, raising=False)
    monkeypatch.setattr(
...
def test_get_recommend_app_detail_should_set_can_trial_when_trial_feature_enabled(
    ...
    retrieval_factory = MagicMock(return_value=retrieval_instance)
    monkeypatch.setattr(service_module.magic_studio_config, "HOSTED_FETCH_APP_TEMPLATES_MODE", "remote", raising=False)
    monkeypatch.setattr(
        service_module.RecommendAppRetrievalFactory,
        "get_recommend_app_factory",
        MagicMock(return_value=retrieval_factory),
