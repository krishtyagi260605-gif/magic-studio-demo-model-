...
class TestBuildStreamingTaskOnSubscribe:
    ...
    def test_streams_mode_starts_immediately(self, monkeypatch):
        monkeypatch.setattr(ags_module.magic_studio_config, "PUBSUB_REDIS_CHANNEL_TYPE", "streams")
        called = []
    ...
    def test_pubsub_mode_starts_on_subscribe(self, monkeypatch):
        monkeypatch.setattr(ags_module.magic_studio_config, "PUBSUB_REDIS_CHANNEL_TYPE", "pubsub")
    ...
    def test_sharded_mode_starts_on_subscribe(self, monkeypatch):
        ...
        monkeypatch.setattr(ags_module.magic_studio_config, "PUBSUB_REDIS_CHANNEL_TYPE", "sharded")
...
class TestGetMaxActiveRequests:
    def test_both_zero_returns_zero(self, monkeypatch):
        monkeypatch.setattr(ags_module.magic_studio_config, "APP_MAX_ACTIVE_REQUESTS", 0)
        monkeypatch.setattr(ags_module.magic_studio_config, "APP_DEFAULT_ACTIVE_REQUESTS", 0)
    ...
    def test_config_limit_only(self, monkeypatch):
        monkeypatch.setattr(ags_module.magic_studio_config, "APP_MAX_ACTIVE_REQUESTS", 10)
        monkeypatch.setattr(ags_module.magic_studio_config, "APP_DEFAULT_ACTIVE_REQUESTS", 0)
        app = _make_app(AppMode.CHAT, max_active_requests=0)
        assert AppGenerateService._get_max_active_requests(app) == 10

    def test_both_non_zero_returns_min(self, monkeypatch):
        monkeypatch.setattr(ags_module.magic_studio_config, "APP_MAX_ACTIVE_REQUESTS", 20)
        monkeypatch.setattr(ags_module.magic_studio_config, "APP_DEFAULT_ACTIVE_REQUESTS", 0)
        app = _make_app(AppMode.CHAT, max_active_requests=5)
        assert AppGenerateService._get_max_active_requests(app) == 5
    ...
    def test_default_active_requests_used_when_app_has_none(self, monkeypatch):
        monkeypatch.setattr(ags_module.magic_studio_config, "APP_MAX_ACTIVE_REQUESTS", 0)
        monkeypatch.setattr(ags_module.magic_studio_config, "APP_DEFAULT_ACTIVE_REQUESTS", 15)
        app = _make_app(AppMode.CHAT, max_active_requests=0)
...
class TestGenerate:
    ...
    def _common(self, mocker, monkeypatch):
        monkeypatch.setattr(ags_module.magic_studio_config, "BILLING_ENABLED", False)
...
class TestGenerateBilling:
    ...
    def test_billing_enabled_consumes_quota(self, mocker, monkeypatch):
        monkeypatch.setattr(ags_module.magic_studio_config, "BILLING_ENABLED", True)
