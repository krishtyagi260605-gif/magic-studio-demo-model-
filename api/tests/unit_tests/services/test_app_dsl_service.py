...
def test_encrypt_decrypt_dataset_id_respects_config(monkeypatch):
    ...
    monkeypatch.setattr(app_dsl_service.magic_studio_config, "DSL_EXPORT_ENCRYPT_DATASET_ID", False)
    assert AppDslService.encrypt_dataset_id(dataset_id=dataset_uuid, tenant_id=tenant_id) == dataset_uuid

    monkeypatch.setattr(app_dsl_service.magic_studio_config, "DSL_EXPORT_ENCRYPT_DATASET_ID", True)
