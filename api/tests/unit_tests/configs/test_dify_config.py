...
from configs.app_config import MagicStudioConfig


def test_magic_studio_config(monkeypatch: pytest.MonkeyPatch):
    ...
    config = MagicStudioConfig(_env_file=None)
...
def test_flask_configs(monkeypatch: pytest.MonkeyPatch):
    ...
    # Disable `.env` loading to ensure test stability across environments
    flask_app.config.from_mapping(MagicStudioConfig(_env_file=None).model_dump())  # pyright: ignore
...
def test_pubsub_redis_url_required_when_default_unavailable(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(ValueError, match="PUBSUB_REDIS_URL must be set"):
        _ = MagicStudioConfig().normalized_pubsub_redis_url
