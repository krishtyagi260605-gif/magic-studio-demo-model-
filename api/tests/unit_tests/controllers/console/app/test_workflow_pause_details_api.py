...
def _patch_console_guards(monkeypatch: pytest.MonkeyPatch, account: Account) -> None:
    monkeypatch.setattr(login_lib.magic_studio_config, "LOGIN_DISABLED", True)
    ...
    monkeypatch.setattr(console_wraps.magic_studio_config, "EDITION", "CLOUD")
...
def test_pause_details_returns_backstage_input_url(app: Flask, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(workflow_run_module.magic_studio_config, "APP_WEB_URL", "https://web.example.com")
