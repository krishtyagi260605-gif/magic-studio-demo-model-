...
def test_real_cookie_name_uses_host_prefix_without_domain(monkeypatch):
    monkeypatch.setattr(token.magic_studio_config, "CONSOLE_WEB_URL", "https://console.example.com", raising=False)
    monkeypatch.setattr(token.magic_studio_config, "CONSOLE_API_URL", "https://api.example.com", raising=False)
    monkeypatch.setattr(token.magic_studio_config, "COOKIE_DOMAIN", "", raising=False)
...
def test_real_cookie_name_without_host_prefix_when_domain_present(monkeypatch):
    monkeypatch.setattr(token.magic_studio_config, "CONSOLE_WEB_URL", "https://console.example.com", raising=False)
    monkeypatch.setattr(token.magic_studio_config, "CONSOLE_API_URL", "https://api.example.com", raising=False)
    monkeypatch.setattr(token.magic_studio_config, "COOKIE_DOMAIN", ".example.com", raising=False)
