...
class AliyunOssStorage(BaseStorage):
    ...
    def __init__(self):
        super().__init__()
        self.bucket_name = magic_studio_config.ALIYUN_OSS_BUCKET_NAME
        self.folder = magic_studio_config.ALIYUN_OSS_PATH
        oss_auth_method = aliyun_s3.Auth
        region = None
        if magic_studio_config.ALIYUN_OSS_AUTH_VERSION == "v4":
            oss_auth_method = aliyun_s3.AuthV4
            region = magic_studio_config.ALIYUN_OSS_REGION
        oss_auth = oss_auth_method(magic_studio_config.ALIYUN_OSS_ACCESS_KEY, magic_studio_config.ALIYUN_OSS_SECRET_KEY)
        self.client = aliyun_s3.Bucket(
            oss_auth,
            magic_studio_config.ALIYUN_OSS_ENDPOINT,
            self.bucket_name,
            connect_timeout=30,
            region=region,
            cloudbox_id=magic_studio_config.ALIYUN_CLOUDBOX_ID,
        )
