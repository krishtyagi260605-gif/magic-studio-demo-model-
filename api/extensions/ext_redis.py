...
def _get_ssl_configuration() -> tuple[type[Union[Connection, SSLConnection]], dict[str, Any]]:
    ...
    if not magic_studio_config.REDIS_USE_SSL:
        return Connection, {}
...
def _get_cache_configuration() -> CacheConfig | None:
    ...
    if not magic_studio_config.REDIS_ENABLE_CLIENT_SIDE_CACHE:
        return None

    resp_protocol = magic_studio_config.REDIS_SERIALIZATION_PROTOCOL
    if resp_protocol < 3:
...
def _get_base_redis_params() -> dict[str, Any]:
    ...
    return {
        "username": magic_studio_config.REDIS_USERNAME,
        "password": magic_studio_config.REDIS_PASSWORD or None,
        "db": magic_studio_config.REDIS_DB,
        "encoding": "utf-8",
        "encoding_errors": "strict",
        "decode_responses": False,
        "protocol": magic_studio_config.REDIS_SERIALIZATION_PROTOCOL,
...
def _create_sentinel_client(redis_params: dict[str, Any]) -> Union[redis.Redis, RedisCluster]:
    ...
    if not magic_studio_config.REDIS_SENTINELS:
        raise ValueError("REDIS_SENTINELS must be set when REDIS_USE_SENTINEL is True")

    if not magic_studio_config.REDIS_SENTINEL_SERVICE_NAME:
        raise ValueError("REDIS_SENTINEL_SERVICE_NAME must be set when REDIS_USE_SENTINEL is True")

    sentinel_hosts = [(node.split(":")[0], int(node.split(":")[1])) for node in magic_studio_config.REDIS_SENTINELS.split(",")]

    sentinel_kwargs = {
        "socket_timeout": magic_studio_config.REDIS_SENTINEL_SOCKET_TIMEOUT,
        "username": magic_studio_config.REDIS_SENTINEL_USERNAME,
        "password": magic_studio_config.REDIS_SENTINEL_PASSWORD,
    }

    if magic_studio_config.REDIS_MAX_CONNECTIONS:
        sentinel_kwargs["max_connections"] = magic_studio_config.REDIS_MAX_CONNECTIONS
    ...
    master: redis.Redis = sentinel.master_for(magic_studio_config.REDIS_SENTINEL_SERVICE_NAME, **redis_params)
    return master


def _create_cluster_client() -> Union[redis.Redis, RedisCluster]:
    """Create Redis cluster client."""
    if not magic_studio_config.REDIS_CLUSTERS:
        raise ValueError("REDIS_CLUSTERS must be set when REDIS_USE_CLUSTERS is True")

    nodes = [
        ClusterNode(host=node.split(":")[0], port=int(node.split(":")[1]))
        for node in magic_studio_config.REDIS_CLUSTERS.split(",")
    ]

    cluster_kwargs: dict[str, Any] = {
        "startup_nodes": nodes,
        "password": magic_studio_config.REDIS_CLUSTERS_PASSWORD,
        "protocol": magic_studio_config.REDIS_SERIALIZATION_PROTOCOL,
        "cache_config": _get_cache_configuration(),
    }
    if magic_studio_config.REDIS_MAX_CONNECTIONS:
        cluster_kwargs["max_connections"] = magic_studio_config.REDIS_MAX_CONNECTIONS
...
def _create_standalone_client(redis_params: dict[str, Any]) -> Union[redis.Redis, RedisCluster]:
    ...
    redis_params.update(
        {
            "host": magic_studio_config.REDIS_HOST,
            "port": magic_studio_config.REDIS_PORT,
            "connection_class": connection_class,
        }
    )

    if magic_studio_config.REDIS_MAX_CONNECTIONS:
        redis_params["max_connections"] = magic_studio_config.REDIS_MAX_CONNECTIONS
...
def _create_pubsub_client(pubsub_url: str, use_clusters: bool) -> redis.Redis | RedisCluster:
    max_conns = magic_studio_config.REDIS_MAX_CONNECTIONS
    if use_clusters:
        if max_conns:
...
def init_app(app: MagicStudioApp):
    ...
    if magic_studio_config.REDIS_USE_SENTINEL:
        redis_params = _get_base_redis_params()
        client = _create_sentinel_client(redis_params)
    elif magic_studio_config.REDIS_USE_CLUSTERS:
    ...
    if magic_studio_config.normalized_pubsub_redis_url:
        _pubsub_redis_client = _create_pubsub_client(
            magic_studio_config.normalized_pubsub_redis_url, magic_studio_config.PUBSUB_REDIS_USE_CLUSTERS
        )
...
def get_pubsub_broadcast_channel() -> BroadcastChannelProtocol:
    ...
    if magic_studio_config.PUBSUB_REDIS_CHANNEL_TYPE == "sharded":
        return ShardedRedisBroadcastChannel(_pubsub_redis_client)
    if magic_studio_config.PUBSUB_REDIS_CHANNEL_TYPE == "streams":
        return StreamsBroadcastChannel(
            _pubsub_redis_client,
            retention_seconds=magic_studio_config.PUBSUB_STREAMS_RETENTION_SECONDS,
        )
