"""Logging extension for the Magic Studio Flask application."""
...
def init_app(app: MagicStudioApp):
    ...
    # File handler
    log_file = magic_studio_config.LOG_FILE
    if log_file:
        ...
        log_handlers.append(
            RotatingFileHandler(
                filename=log_file,
                maxBytes=magic_studio_config.LOG_FILE_MAX_SIZE * 1024 * 1024,
                backupCount=magic_studio_config.LOG_FILE_BACKUP_COUNT,
            )
        )
    ...
    logging.basicConfig(
        level=magic_studio_config.LOG_LEVEL,
    ...
    if magic_studio_config.LOG_OUTPUT_FORMAT == "text":
        _apply_timezone(log_handlers)


def _create_formatter() -> logging.Formatter:
    """Create appropriate formatter based on configuration."""
    if magic_studio_config.LOG_OUTPUT_FORMAT == "json":
        ...
        return _TextFormatter(
            fmt=magic_studio_config.LOG_FORMAT,
            datefmt=magic_studio_config.LOG_DATEFORMAT,
        )


def _apply_timezone(handlers: list[logging.Handler]):
    """Apply timezone conversion to text formatters."""
    log_tz = magic_studio_config.LOG_TZ
    if log_tz:
...
def apply_request_id_formatter():
    ...
    for handler in logging.root.handlers:
        if handler.formatter:
            handler.formatter = RequestIdFormatter(magic_studio_config.LOG_FORMAT, magic_studio_config.LOG_DATEFORMAT)
