...
def init_app(app: MagicStudioApp):
    ...
    def after_request(response):  # pyright: ignore[reportUnusedFunction]
        ...
        response.headers.add("X-Version", magic_studio_config.project.version)
        response.headers.add("X-Env", magic_studio_config.DEPLOY_ENV)
        return response

    @app.route("/health")
    def health():  # pyright: ignore[reportUnusedFunction]
        return Response(
            json.dumps({"pid": os.getpid(), "status": "ok", "version": magic_studio_config.project.version}),
            status=200,
