...
plugin_daemon_inner_api_baseurl = URL(str(magic_studio_config.PLUGIN_DAEMON_URL))
_plugin_daemon_timeout_config = cast(
    float | httpx.Timeout | None,
    getattr(magic_studio_config, "PLUGIN_DAEMON_TIMEOUT", 600.0),
)
...
class BasePluginClient:
    ...
    def _prepare_request(
        ...
        prepared_headers["X-Api-Key"] = magic_studio_config.PLUGIN_DAEMON_KEY
    ...
    def _inject_trace_headers(self, headers: dict[str, str]) -> None:
        ...
        """
        if not magic_studio_config.ENABLE_OTEL:
            return
    ...
    def _request_with_plugin_daemon_response[T: BaseModel | dict[str, Any] | list[Any] | bool | str](
        ...
        try:
            ...
            # Graceful fallback for specific informational paths when daemon is unreachable
            if any(p in path for p in ["/management/install/tasks", "/management/list"]):
                logger.warning("Plugin daemon unreachable for path %s, returning empty fallback.", path)
                if inspect.isclass(type_) and issubclass(type_, list):
                    return cast(T, [])
                if type_ == bool:
                    return cast(T, False)
                # For pydantic models or other types, try to return a default if possible
                # or just re-raise if it's a critical path. 
                # For tasks, it's usually a list.
                if "tasks" in path:
                    return cast(T, [])
