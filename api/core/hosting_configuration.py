...
class HostingConfiguration:
    ...
    def init_app(self, app: Flask):
        if magic_studio_config.EDITION != "CLOUD":
            return
    ...
    def init_azure_openai() -> HostingProvider:
        ...
        if magic_studio_config.HOSTED_AZURE_OPENAI_ENABLED:
            credentials = {
                "openai_api_key": magic_studio_config.HOSTED_AZURE_OPENAI_API_KEY,
                "openai_api_base": magic_studio_config.HOSTED_AZURE_OPENAI_API_BASE,
                "base_model_name": "gpt-35-turbo",
            }

            quotas: list[HostingQuota] = []
            hosted_quota_limit = magic_studio_config.HOSTED_AZURE_OPENAI_QUOTA_LIMIT
    ...
    def init_openai(self) -> HostingProvider:
        ...
        if magic_studio_config.HOSTED_OPENAI_TRIAL_ENABLED:
        ...
        if magic_studio_config.HOSTED_OPENAI_PAID_ENABLED:
        ...
        if len(quotas) > 0:
            credentials = {
                "openai_api_key": magic_studio_config.HOSTED_OPENAI_API_KEY,
            }

            if magic_studio_config.HOSTED_OPENAI_API_BASE:
                credentials["openai_api_base"] = magic_studio_config.HOSTED_OPENAI_API_BASE

            if magic_studio_config.HOSTED_OPENAI_API_ORGANIZATION:
                credentials["openai_organization"] = magic_studio_config.HOSTED_OPENAI_API_ORGANIZATION
    ...
    def init_gemini(self) -> HostingProvider:
        ...
        if magic_studio_config.HOSTED_GEMINI_TRIAL_ENABLED:
            hosted_quota_limit = 0
            trial_models = self.parse_restrict_models_from_env("HOSTED_GEMINI_TRIAL_MODELS")
            trial_quota = TrialHostingQuota(quota_limit=hosted_quota_limit, restrict_models=trial_models)
            quotas.append(trial_quota)

        if magic_studio_config.HOSTED_GEMINI_PAID_ENABLED:
        ...
        if len(quotas) > 0:
            credentials = {
                "google_api_key": magic_studio_config.HOSTED_GEMINI_API_KEY,
            }

            if magic_studio_config.HOSTED_GEMINI_API_BASE:
                credentials["google_base_url"] = magic_studio_config.HOSTED_GEMINI_API_BASE
    ...
    def init_anthropic(self) -> HostingProvider:
        ...
        if magic_studio_config.HOSTED_ANTHROPIC_TRIAL_ENABLED:
            hosted_quota_limit = 0
        ...
        if magic_studio_config.HOSTED_ANTHROPIC_PAID_ENABLED:
        ...
        if len(quotas) > 0:
            credentials = {
                "anthropic_api_key": magic_studio_config.HOSTED_ANTHROPIC_API_KEY,
            }

            if magic_studio_config.HOSTED_ANTHROPIC_API_BASE:
                credentials["anthropic_api_url"] = magic_studio_config.HOSTED_ANTHROPIC_API_BASE
    ...
    def init_tongyi(self) -> HostingProvider:
        ...
        if magic_studio_config.HOSTED_TONGYI_TRIAL_ENABLED:
            hosted_quota_limit = 0
            trail_models = self.parse_restrict_models_from_env("HOSTED_TONGYI_TRIAL_MODELS")
            trial_quota = TrialHostingQuota(quota_limit=hosted_quota_limit, restrict_models=trail_models)
            quotas.append(trial_quota)

        if magic_studio_config.HOSTED_TONGYI_PAID_ENABLED:
        ...
        if len(quotas) > 0:
            credentials = {
                "dashscope_api_key": magic_studio_config.HOSTED_TONGYI_API_KEY,
                "use_international_endpoint": magic_studio_config.HOSTED_TONGYI_USE_INTERNATIONAL_ENDPOINT,
            }
    ...
    def init_xai(self) -> HostingProvider:
        ...
        if magic_studio_config.HOSTED_XAI_TRIAL_ENABLED:
            hosted_quota_limit = 0
            trail_models = self.parse_restrict_models_from_env("HOSTED_XAI_TRIAL_MODELS")
            trial_quota = TrialHostingQuota(quota_limit=hosted_quota_limit, restrict_models=trail_models)
            quotas.append(trial_quota)

        if magic_studio_config.HOSTED_XAI_PAID_ENABLED:
        ...
        if len(quotas) > 0:
            credentials = {
                "api_key": magic_studio_config.HOSTED_XAI_API_KEY,
            }

            if magic_studio_config.HOSTED_XAI_API_BASE:
                credentials["endpoint_url"] = magic_studio_config.HOSTED_XAI_API_BASE
    ...
    def init_deepseek(self) -> HostingProvider:
        ...
        if magic_studio_config.HOSTED_DEEPSEEK_TRIAL_ENABLED:
            hosted_quota_limit = 0
            trail_models = self.parse_restrict_models_from_env("HOSTED_DEEPSEEK_TRIAL_MODELS")
            trial_quota = TrialHostingQuota(quota_limit=hosted_quota_limit, restrict_models=trail_models)
            quotas.append(trial_quota)

        if magic_studio_config.HOSTED_DEEPSEEK_PAID_ENABLED:
        ...
        if len(quotas) > 0:
            credentials = {
                "api_key": magic_studio_config.HOSTED_DEEPSEEK_API_KEY,
            }

            if magic_studio_config.HOSTED_DEEPSEEK_API_BASE:
                credentials["endpoint_url"] = magic_studio_config.HOSTED_DEEPSEEK_API_BASE
    ...
    def init_minimax() -> HostingProvider:
        ...
        if magic_studio_config.HOSTED_MINIMAX_ENABLED:
    ...
    def init_spark() -> HostingProvider:
        quota_unit = QuotaUnit.TOKENS
        if magic_studio_config.HOSTED_SPARK_ENABLED:
            quotas: list[HostingQuota] = [FreeHostingQuota()]
    ...
    def init_zhipuai() -> HostingProvider:
        quota_unit = QuotaUnit.TOKENS
        if magic_studio_config.HOSTED_ZHIPUAI_ENABLED:
            quotas: list[HostingQuota] = [FreeHostingQuota()]
    ...
    def init_moderation_config() -> HostedModerationConfig:
        if magic_studio_config.HOSTED_MODERATION_ENABLED and magic_studio_config.HOSTED_MODERATION_PROVIDERS:
            providers = magic_studio_config.HOSTED_MODERATION_PROVIDERS.split(",")
            hosted_providers = []
    ...
    def parse_restrict_models_from_env(env_var: str) -> list[RestrictModel]:
        models_str = magic_studio_config.model_dump().get(env_var)
