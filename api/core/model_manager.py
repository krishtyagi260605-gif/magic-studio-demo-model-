...
class ModelInstance:
    ...
    def invoke_with_fallback(
        ...
        **kwargs: Any,
        ...
        """
        Invoke the model with a fallback chain.
        """
        from core.model_runtime.model_fallback import FallbackModelConfig, ModelFallbackChain

        chain_configs = [
            FallbackModelConfig(provider=self.provider, model_name=self.model_name, priority=0)
        ]
        ...
        chain = ModelFallbackChain(models=chain_configs)

        def runner(provider: str, model_name: str, *r_args, **r_kwargs):
            if provider == self.provider and model_name == self.model_name:
                return self.invoke_llm(*r_args, **r_kwargs)
            # For fallback models, we would need to get their instances
            # This is a simplified implementation for Magic Studio demonstration
            return self.invoke_llm(*r_args, **r_kwargs)

        return chain.invoke(runner, *args, **kwargs)
