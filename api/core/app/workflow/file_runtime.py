...
class DifyWorkflowFileRuntime(WorkflowFileRuntimeProtocol):
    ...
    def multimodal_send_format(self) -> str:
        return magic_studio_config.MULTIMODAL_SEND_FORMAT
    ...
    def verify_preview_signature(
        ...
        *,
        ...
        if sign != base64.urlsafe_b64encode(recalculated).decode():
            return False
        return int(time.time()) - int(timestamp) <= magic_studio_config.FILES_ACCESS_TIMEOUT

    @staticmethod
    def _base_url(*, for_external: bool) -> str:
        if for_external:
            return magic_studio_config.FILES_URL
        return magic_studio_config.INTERNAL_FILES_URL or magic_studio_config.FILES_URL

    @staticmethod
    def _secret_key() -> bytes:
        return magic_studio_config.SECRET_KEY.encode() if magic_studio_config.SECRET_KEY else b""
