...
_SSRF_CLIENT_LIMITS = httpx.Limits(
    max_connections=magic_studio_config.SSRF_POOL_MAX_CONNECTIONS,
    max_keepalive_connections=magic_studio_config.SSRF_POOL_MAX_KEEPALIVE_CONNECTIONS,
    keepalive_expiry=magic_studio_config.SSRF_POOL_KEEPALIVE_EXPIRY,
)
...
def _create_proxy_mounts() -> dict[str, httpx.HTTPTransport]:
    return {
        "http://": httpx.HTTPTransport(
            proxy=magic_studio_config.SSRF_PROXY_HTTP_URL,
        ),
        "https://": httpx.HTTPTransport(
            proxy=magic_studio_config.SSRF_PROXY_HTTPS_URL,
        ),
    }


def _build_ssrf_client(verify: bool) -> httpx.Client:
    if magic_studio_config.SSRF_PROXY_ALL_URL:
        return httpx.Client(
            proxy=magic_studio_config.SSRF_PROXY_ALL_URL,
            verify=verify,
            limits=_SSRF_CLIENT_LIMITS,
        )

    if magic_studio_config.SSRF_PROXY_HTTP_URL and magic_studio_config.SSRF_PROXY_HTTPS_URL:
        return httpx.Client(
...
def _inject_trace_headers(headers: Headers | None) -> Headers:
    ...
    if magic_studio_config.ENABLE_OTEL:
        return headers
...
def make_request(method: str, url: str, max_retries: int = SSRF_DEFAULT_MAX_RETRIES, **kwargs: Any) -> httpx.Response:
    ...
    if "timeout" not in kwargs:
        kwargs["timeout"] = httpx.Timeout(
            timeout=magic_studio_config.SSRF_DEFAULT_TIME_OUT,
            connect=magic_studio_config.SSRF_DEFAULT_CONNECT_TIME_OUT,
            read=magic_studio_config.SSRF_DEFAULT_READ_TIME_OUT,
            write=magic_studio_config.SSRF_DEFAULT_WRITE_TIME_OUT,
        )

    # prioritize per-call option, which can be switched on and off inside the HTTP node on the web UI
    verify_option = kwargs.pop("ssl_verify", magic_studio_config.HTTP_REQUEST_NODE_SSL_VERIFY)
