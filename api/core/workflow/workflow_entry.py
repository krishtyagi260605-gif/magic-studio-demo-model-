...
class WorkflowEntry:
    def __init__(
        ...
        """
        # check call depth
        workflow_call_max_depth = magic_studio_config.WORKFLOW_CALL_MAX_DEPTH
        ...
        self.graph_engine = GraphEngine(
            ...
            command_channel=command_channel,
            config=GraphEngineConfig(
                min_workers=magic_studio_config.GRAPH_ENGINE_MIN_WORKERS,
                max_workers=magic_studio_config.GRAPH_ENGINE_MAX_WORKERS,
                scale_up_threshold=magic_studio_config.GRAPH_ENGINE_SCALE_UP_THRESHOLD,
                scale_down_idle_time=magic_studio_config.GRAPH_ENGINE_SCALE_DOWN_IDLE_TIME,
            ),
        ...
        limits_layer = ExecutionLimitsLayer(
            max_steps=magic_studio_config.WORKFLOW_MAX_EXECUTION_STEPS, max_time=magic_studio_config.WORKFLOW_MAX_EXECUTION_TIME
        )
        self.graph_engine.layer(limits_layer)
        self.graph_engine.layer(LLMQuotaLayer())

        # Add observability layer when OTel is enabled
        if magic_studio_config.ENABLE_OTEL or is_instrument_flag_enabled():
