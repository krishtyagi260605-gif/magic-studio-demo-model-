...
class RelytVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> RelytVector:
        ...
        return RelytVector(
            ...
            config=RelytConfig(
                host=magic_studio_config.RELYT_HOST or "localhost",
                port=magic_studio_config.RELYT_PORT,
                user=magic_studio_config.RELYT_USER or "",
                password=magic_studio_config.RELYT_PASSWORD or "",
                database=magic_studio_config.RELYT_DATABASE or "default",
            ),
            group_id=dataset.id,
        )
