        ...
        with redis_client.lock(lock_name, timeout=20):
            ...
            try:
                if not self._client.collections.exists(self._collection_name):
                    tokenization = (
                        wc.Tokenization(magic_studio_config.WEAVIATE_TOKENIZATION)
                        if magic_studio_config.WEAVIATE_TOKENIZATION
    ...
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> WeaviateVector:
        ...
        return WeaviateVector(
            ...
            config=WeaviateConfig(
                endpoint=magic_studio_config.WEAVIATE_ENDPOINT or "",
                grpc_endpoint=magic_studio_config.WEAVIATE_GRPC_ENDPOINT or "",
                api_key=magic_studio_config.WEAVIATE_API_KEY,
                batch_size=magic_studio_config.WEAVIATE_BATCH_SIZE,
            ),
