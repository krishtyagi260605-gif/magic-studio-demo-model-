...
class AnalyticdbVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> AnalyticdbVector:
        ...
        if magic_studio_config.ANALYTICDB_HOST is None:
            # implemented through OpenAPI
            apiConfig = AnalyticdbVectorOpenAPIConfig(
                access_key_id=magic_studio_config.ANALYTICDB_KEY_ID or "",
                access_key_secret=magic_studio_config.ANALYTICDB_KEY_SECRET or "",
                region_id=magic_studio_config.ANALYTICDB_REGION_ID or "",
                instance_id=magic_studio_config.ANALYTICDB_INSTANCE_ID or "",
                account=magic_studio_config.ANALYTICDB_ACCOUNT or "",
                account_password=magic_studio_config.ANALYTICDB_PASSWORD or "",
                namespace=magic_studio_config.ANALYTICDB_NAMESPACE or "",
                namespace_password=magic_studio_config.ANALYTICDB_NAMESPACE_PASSWORD,
            )
            sqlConfig = None
        else:
            # implemented through sql
            sqlConfig = AnalyticdbVectorBySqlConfig(
                host=magic_studio_config.ANALYTICDB_HOST,
                port=magic_studio_config.ANALYTICDB_PORT,
                account=magic_studio_config.ANALYTICDB_ACCOUNT or "",
                account_password=magic_studio_config.ANALYTICDB_PASSWORD or "",
                min_connection=magic_studio_config.ANALYTICDB_MIN_CONNECTION,
                max_connection=magic_studio_config.ANALYTICDB_MAX_CONNECTION,
                namespace=magic_studio_config.ANALYTICDB_NAMESPACE or "",
            )
            apiConfig = None
