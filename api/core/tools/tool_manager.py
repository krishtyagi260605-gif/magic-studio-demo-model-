...
class ToolManager:
    ...
    def get_tool_runtime(
        ...
        if provider_type == ToolProviderType.BUILT_IN:
            ...
            if builtin_provider.expires_at != -1 and (builtin_provider.expires_at - 60) < int(time.time()):
                ...
                redirect_uri = f"{magic_studio_config.CONSOLE_API_URL}/console/api/oauth/plugin/{provider_id}/tool/callback"
    ...
    def list_default_builtin_providers(cls, tenant_id: str) -> list[BuiltinToolProvider]:
        ...
        if magic_studio_config.SQLALCHEMY_DATABASE_URI_SCHEME == "postgresql":
        ...
        with Session(db.engine) as session:
            if "builtin" in filters:
                ...
                for provider in builtin_providers:
                    ...
                    if is_filtered(
                        include_set=magic_studio_config.POSITION_TOOL_INCLUDES_SET,
                        exclude_set=magic_studio_config.POSITION_TOOL_EXCLUDES_SET,
                        data=provider,
    ...
    def generate_builtin_tool_icon_url(cls, provider_id: str) -> str:
        return str(
            URL(magic_studio_config.CONSOLE_API_URL or "/")
            / "console"
            / "api"
            / "workspaces"
            / "current"
