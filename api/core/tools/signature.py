...
def sign_tool_file(tool_file_id: str, extension: str, for_external: bool = True) -> str:
    ...
    base_url = magic_studio_config.FILES_URL if for_external else (magic_studio_config.INTERNAL_FILES_URL or magic_studio_config.FILES_URL)
    file_preview_url = f"{base_url}/files/tools/{tool_file_id}{extension}"

    timestamp = str(int(time.time()))
    nonce = os.urandom(16).hex()
    data_to_sign = f"file-preview|{tool_file_id}|{timestamp}|{nonce}"
    secret_key = magic_studio_config.SECRET_KEY.encode() if magic_studio_config.SECRET_KEY else b""
...
def verify_tool_file_signature(file_id: str, timestamp: str, nonce: str, sign: str) -> bool:
    ...
    return current_time - int(timestamp) <= magic_studio_config.FILES_ACCESS_TIMEOUT
