...
def is_secure() -> bool:
    return magic_studio_config.CONSOLE_WEB_URL.startswith("https") and magic_studio_config.CONSOLE_API_URL.startswith("https")
...
def _cookie_domain() -> str | None:
    ...
    """
    domain = magic_studio_config.COOKIE_DOMAIN.strip()
...
def set_access_token_to_cookie(request: Request, response: Response, token: str, samesite: str = "Lax"):
    response.set_cookie(
        ...
        secure=is_secure(),
        samesite=samesite,
        max_age=int(magic_studio_config.ACCESS_TOKEN_EXPIRE_MINUTES * 60),
        path="/",
    )
...
def set_refresh_token_to_cookie(request: Request, response: Response, token: str):
    response.set_cookie(
        ...
        secure=is_secure(),
        samesite="Lax",
        max_age=int(60 * 60 * 24 * magic_studio_config.REFRESH_TOKEN_EXPIRE_DAYS),
        path="/",
    )
...
def set_csrf_token_to_cookie(request: Request, response: Response, token: str):
    response.set_cookie(
        ...
        secure=is_secure(),
        samesite="Lax",
        max_age=int(60 * magic_studio_config.ACCESS_TOKEN_EXPIRE_MINUTES),
        path="/",
    )
...
def check_csrf_token(request: Request, user_id: str):
    ...
    if magic_studio_config.ADMIN_API_KEY_ENABLE:
        auth_token = extract_access_token(request)
        if auth_token and auth_token == magic_studio_config.ADMIN_API_KEY:
            return
