...
def convert_datetime_to_date(field, target_timezone: str = ":tz"):
    if magic_studio_config.DB_TYPE == "postgresql":
        return f"DATE(DATE_TRUNC('day', {field} AT TIME ZONE 'UTC' AT TIME ZONE {target_timezone}))"
    elif magic_studio_config.DB_TYPE in ["mysql", "oceanbase", "seekdb"]:
        return f"DATE(CONVERT_TZ({field}, 'UTC', {target_timezone}))"
    else:
        raise NotImplementedError(f"Unsupported database type: {magic_studio_config.DB_TYPE}")
...
class TokenManager:
    ...
    def generate_token(
        ...
        expiry_minutes = magic_studio_config.model_dump().get(f"{token_type.upper()}_TOKEN_EXPIRY_MINUTES")
