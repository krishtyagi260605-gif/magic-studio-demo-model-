...
class ArchiveStorage:
    ...
    def __init__(self, bucket: str):
        if not magic_studio_config.ARCHIVE_STORAGE_ENABLED:
        ...
        if not all(
            [
                magic_studio_config.ARCHIVE_STORAGE_ENDPOINT,
                bucket,
                magic_studio_config.ARCHIVE_STORAGE_ACCESS_KEY,
                magic_studio_config.ARCHIVE_STORAGE_SECRET_KEY,
            ]
        ):
        ...
        self.client = boto3.client(
            "s3",
            endpoint_url=magic_studio_config.ARCHIVE_STORAGE_ENDPOINT,
            aws_access_key_id=magic_studio_config.ARCHIVE_STORAGE_ACCESS_KEY,
            aws_secret_access_key=magic_studio_config.ARCHIVE_STORAGE_SECRET_KEY,
            region_name=magic_studio_config.ARCHIVE_STORAGE_REGION,
            config=Config(
...
def get_archive_storage() -> ArchiveStorage:
    ...
    if _archive_storage is None:
        archive_bucket = magic_studio_config.ARCHIVE_STORAGE_ARCHIVE_BUCKET
...
def get_export_storage() -> ArchiveStorage:
    ...
    if _export_storage is None:
        export_bucket = magic_studio_config.ARCHIVE_STORAGE_EXPORT_BUCKET
