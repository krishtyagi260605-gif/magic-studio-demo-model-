...
redis_config = parse_url(magic_studio_config.CELERY_BROKER_URL)
celery_redis = Redis(
    ...
    ssl=bool(magic_studio_config.BROKER_USE_SSL),
    ssl_ca_certs=magic_studio_config.REDIS_SSL_CA_CERTS if magic_studio_config.BROKER_USE_SSL else None,
    ssl_cert_reqs=getattr(magic_studio_config, "REDIS_SSL_CERT_REQS", None) if magic_studio_config.BROKER_USE_SSL else None,
    ssl_certfile=getattr(magic_studio_config, "REDIS_SSL_CERTFILE", None) if magic_studio_config.BROKER_USE_SSL else None,
    ssl_keyfile=getattr(magic_studio_config, "REDIS_SSL_KEYFILE", None) if magic_studio_config.BROKER_USE_SSL else None,
...
def queue_monitor_task():
    queue_name = "dataset"
    threshold = magic_studio_config.QUEUE_MONITOR_THRESHOLD
    ...
    try:
        ...
        if queue_length >= threshold:
            ...
            alert_emails = magic_studio_config.QUEUE_MONITOR_ALERT_EMAILS
            if alert_emails:
