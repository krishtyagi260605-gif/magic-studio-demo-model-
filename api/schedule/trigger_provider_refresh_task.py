...
def _build_due_filter(now_ts: int):
    ...
    credential_due: ColumnElement[bool] = and_(
        ...
        <= now_ts + int(magic_studio_config.TRIGGER_PROVIDER_CREDENTIAL_THRESHOLD_SECONDS),
    )
    subscription_due: ColumnElement[bool] = and_(
        TriggerSubscription.expires_at != -1,
        TriggerSubscription.expires_at <= now_ts + int(magic_studio_config.TRIGGER_PROVIDER_SUBSCRIPTION_THRESHOLD_SECONDS),
    )
...
def trigger_provider_refresh() -> None:
    ...
    batch_size: int = int(magic_studio_config.TRIGGER_PROVIDER_REFRESH_BATCH_SIZE)
    lock_ttl: int = max(300, int(magic_studio_config.TRIGGER_PROVIDER_SUBSCRIPTION_THRESHOLD_SECONDS))
