    ...
    with session_factory() as session:
        ...
        while True:
            ...
            with current_app.producer_or_acquire() as producer:  # type: ignore
                ...
                if 0 < magic_studio_config.WORKFLOW_SCHEDULE_MAX_DISPATCH_PER_TICK <= total_dispatched:
                    logger.warning(
                        "Circuit breaker activated: reached dispatch limit (%d), will continue next tick",
                        magic_studio_config.WORKFLOW_SCHEDULE_MAX_DISPATCH_PER_TICK,
                    )
                    break
...
def _fetch_due_schedules(session: Session) -> list[WorkflowSchedulePlan]:
    ...
    due_schedules = session.scalars(
        ...
        .limit(magic_studio_config.WORKFLOW_SCHEDULE_POLLER_BATCH_SIZE)
    )
