...
def clean_messages():
    ...
    try:
        ...
        policy = create_message_clean_policy(
            graceful_period_days=magic_studio_config.SANDBOX_EXPIRED_RECORDS_CLEAN_GRACEFUL_PERIOD,
        )

        # Create and run the cleanup service
        # lock the task to avoid concurrent execution in case of the future data volume growth
        with redis_client.lock(
            "retention:clean_messages", timeout=magic_studio_config.SANDBOX_EXPIRED_RECORDS_CLEAN_TASK_LOCK_TTL, blocking=False
        ):
