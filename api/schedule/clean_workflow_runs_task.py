...
def clean_workflow_runs_task() -> None:
    ...
    click.echo(
        click.style(
            (
                ...
                f"cutoff={magic_studio_config.SANDBOX_EXPIRED_RECORDS_RETENTION_DAYS} days, "
                f"batch={magic_studio_config.SANDBOX_EXPIRED_RECORDS_CLEAN_BATCH_SIZE}"
            ),
            fg="green",
        )
    )
    ...
    try:
        ...
        with redis_client.lock(
            ...
            timeout=magic_studio_config.SANDBOX_EXPIRED_RECORDS_CLEAN_TASK_LOCK_TTL,
            blocking=False,
        ):
            WorkflowRunCleanup(
                days=magic_studio_config.SANDBOX_EXPIRED_RECORDS_RETENTION_DAYS,
                batch_size=magic_studio_config.SANDBOX_EXPIRED_RECORDS_CLEAN_BATCH_SIZE,
                start_from=None,
                end_before=None,
            ).run()
