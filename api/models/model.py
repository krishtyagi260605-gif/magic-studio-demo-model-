...
class App(Base):
    ...
    def api_base_url(self) -> str:
        return (magic_studio_config.SERVICE_API_URL or request.host_url.rstrip("/")) + "/v1"
...
class Site(Base):
    ...
    def app_base_url(self):
        return magic_studio_config.APP_WEB_URL or request.url_root.rstrip("/")
...
class ApiToken(Base):  # bug: this uses setattr so idk the field.
    ...
    workflow_id = mapped_column(StringUUID, nullable=True)  # Magic Studio: Per-workflow API keys
    ...
    # Magic Studio: Cost Guardrails & Rate Limits
    rate_limit_per_minute: Mapped[int | None] = mapped_column(sa.Integer, nullable=True)
    token_budget: Mapped[int | None] = mapped_column(sa.Integer, nullable=True)
    used_tokens: Mapped[int] = mapped_column(sa.Integer, nullable=False, server_default=sa.text("0"), default=0)
